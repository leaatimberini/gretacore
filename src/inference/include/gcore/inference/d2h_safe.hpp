...
inline void audit_d2h_start(hipStream_t stream, const void *dev_ptr,
                            void *host_ptr, size_t bytes,
                            const char *tensor_name, int step, int layer) {
  ...
  fprintf(audit_log,
          ...
          op_id, get_audit_timestamp(), (void *)stream, (unsigned long)dev_ptr,
          (unsigned long)host_ptr, bytes, tensor_name, step, layer);
  fflush(audit_log);
}

/// Registrar fin de operación D2H en modo auditoría
inline void audit_d2h_end(hipStream_t stream, hipError_t result,
                          const char *tensor_name, size_t op_id) {
  ...
  fprintf(audit_log,
          ...
          op_id, get_audit_timestamp(), (void *)stream,
          hipGetErrorString(result), tensor_name);
  fflush(audit_log);
}

/// Wrapper para hipEventRecord + hipEventSynchronize en modo audit
inline bool audit_event_record_sync(hipEvent_t event, hipStream_t stream,
                                    const char *context) {
...
/// B3.64: Añade instrumentación de debug para diagnosis de illegal memory
/// access
inline bool greta_hip_memcpy_d2h_safe(void *dst, const void *src, size_t bytes,
  ...
  if (meta.alloc_bytes > 0) {
    if (meta.offset_bytes + meta.size_bytes > meta.alloc_bytes) {
      std::cerr << "[D2H_BOUNDS] tensor=" << meta.tensor_name
                ...
                << " size=" << meta.size_bytes << " alloc=" << meta.alloc_bytes
                << "\n";
      return false;
    }
  }
  ...
  if (is_debug_mode()) {
    std::cerr << "[D2H_CHECK] tensor=" << meta.tensor_name
              ...
              << " offset=" << meta.offset_bytes << " size=" << meta.size_bytes
  ...
  if (err != hipSuccess) {
    std::cerr << "[D2H_ERROR] hipMemcpyAsync failed: " << hipGetErrorString(err)
              << " tensor=" << meta.tensor_name << "\n";
...
/// access
inline bool
greta_hip_memcpy_d2h_safe_sync(void *dst, const void *src, size_t bytes,
                               hipMemcpyKind kind,
                               const D2HMetadata &meta = D2HMetadata()) {
  ...
  if (is_debug_mode()) {
    hipError_t dev_sync = hipDeviceSynchronize();
    if (dev_sync != hipSuccess) {
      std::cerr << "[D2H_DEBUG] pre-sync error: " << hipGetErrorString(dev_sync)
                << "\n";
    }
  }
  ...
  for (int retry = 0; retry < MAX_RETRIES; ++retry) {
    ...
    if (err == hipErrorIllegalAddress) {
      std::cerr << "[D2H_ERROR] Illegal memory access on " << meta.tensor_name
    ...
    std::cerr << "[D2H_ERROR] Sync copy failed " << meta.tensor_name << ": "
              << hipGetErrorString(err) << "\n";
    return false;
  }

  std::cerr << "[D2H_ERROR] Sync copy failed after retries " << meta.tensor_name
            << "\n";
  return false;
}
